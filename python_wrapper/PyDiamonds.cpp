...
#include "include/PyPrincipalComponentProjector.h"
...
PYBIND11_MODULE(pyDiamonds, m)
{
    py::class_<Clusterer, PyClusterer<>>(m, "Clusterer")
            .def(py::init_alias<Metric &, Projector &, bool>())
    ...
    py::class_<Projector, PyProjector>(m, "Projector")
            .def(py::init<bool>())
            .def("projection", &Projector::projection);


    py::class_<PrincipalComponentProjector, PyPrincipalComponentProjector, Projector>(m, "PrincipalComponentProjector")
            .def(py::init<bool, double>())
            .def("projection", &PrincipalComponentProjector::projection);
    ...
    py::class_<EuclideanMetric, PyImplMetric<EuclideanMetric>, Metric>(m, "EuclideanMetric")
            .def(py::init_alias<>())
    ...
    py::class_<KmeansClusterer, PyImplClusterer<KmeansClusterer>, Clusterer>(m, "KmeansClusterer")
            .def(py::init_alias<Metric &, Projector &, bool,  unsigned int, unsigned int, unsigned int, double>())
...
//      .def("cluster", &ZeroClusterer::cluster);
    ...
    py::class_<ZeroModel, PyModel<ZeroModel>, Model>(m, "ZeroModel")
